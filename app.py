...
from src.gestacion import (
    registrar_gestacion,
    obtener_gestaciones,
    obtener_gestaciones_proximas,
    actualizar_estado_gestacion,
    obtener_gestacion_por_id,
    editar_gestacion,
    eliminar_gestacion
)
...
def dashboard():
    try:
        ...
        total_vacunaciones_pendientes = cursor.fetchone()['total']
        ...
        # Obtener próximas vacunaciones (próximos 30 días)
        cursor.execute("""
            ...
            LIMIT 5
        """)
        proximas_vacunaciones = cursor.fetchall()
...
def recuperar_contrasena():
    ...
    # Importar el sistema de recuperación de contraseñas
    from src.recuperacion_contrasena import RecuperacionContrasena
    sistema_recuperacion = RecuperacionContrasena(app)
    ...
    if request.method == 'POST':
        ...
        usuario = sistema_recuperacion.email_exists(email)
        if usuario:
            # Generar token de recuperación
            token = sistema_recuperacion.generar_token()
            
            # Guardar token en la base de datos
            if sistema_recuperacion.guardar_token(usuario['id'], token):
                # Construir URL base
                ...
                if sistema_recuperacion.enviar_correo_recuperacion(email, usuario['username'], token, base_url):
                    flash('Se ha enviado un correo de recuperación a tu email. Por favor, revisa tu bandeja de entrada.', 'success')
                else:
                    flash('Hubo un problema al enviar el correo. Por favor, contacta al administrador.', 'error')
            else:
                flash('Hubo un problema al procesar tu solicitud. Por favor, intenta de nuevo más tarde.', 'error')
...
@app.route('/obtener_gestacion/<int:gestacion_id>', methods=['GET'])
@login_required
def obtener_gestacion_route(gestacion_id):
    success, result = obtener_gestacion_por_id(gestacion_id)
    
    if success:
        # Convertir fechas a formato string para JSON
        if result['fecha_inseminacion']:
            result['fecha_inseminacion'] = result['fecha_inseminacion'].strftime('%Y-%m-%d')
        if 'fecha_probable_parto' in result and result['fecha_probable_parto']:
            result['fecha_probable_parto'] = result['fecha_probable_parto'].strftime('%Y-%m-%d')
        return jsonify({'success': True, 'gestacion': result})
    else:
        return jsonify({'success': False, 'message': result})

@app.route('/editar_gestacion/<int:gestacion_id>', methods=['POST'])
@login_required
def editar_gestacion_route(gestacion_id):
    fecha_inseminacion = request.form.get('fecha_inseminacion')
    observaciones = request.form.get('observaciones')
    
    success, message = editar_gestacion(
        gestacion_id=gestacion_id,
...
@app.route('/eliminar_gestacion/<int:gestacion_id>', methods=['POST'])
@login_required
def eliminar_gestacion_route(gestacion_id):
...
@app.route('/desparasitacion/eliminar/<int:id>', methods=['DELETE', 'POST'])
@login_required
def eliminar_desparasitacion(id):
    try:
        ...
        # Primero eliminar las relaciones con animales
        ...
        return jsonify({
            'success': True,
            'message': 'Registro de desparasitación eliminado correctamente'
        })
    except Exception as e:
...
@app.route('/vitaminizacion/eliminar/<int:id>', methods=['DELETE', 'POST'])
@login_required
def eliminar_vitaminizacion(id):
    try:
        ...
        return jsonify({
            'success': True,
            'message': 'Registro de vitaminización eliminado correctamente'
        })
    except Exception as e:
...
@app.route('/fiebre_aftosa/eliminar/<int:id>', methods=['DELETE', 'POST'])
@login_required
def eliminar_fiebre_aftosa(id):
    try:
        ...
        return jsonify({
            'success': True,
            'message': 'Registro de fiebre aftosa eliminado correctamente'
        })
    except Exception as e:
        app.logger.error(f'Error al eliminar fiebre aftosa: {str(e)}')
        conn.rollback()
        cursor.close()
        conn.close()
        return jsonify({
...
@app.route('/carbunco/eliminar/<int:id>', methods=['DELETE', 'POST'])
@login_required
def eliminar_carbunco(id):
    try:
        ...
        return jsonify({
            'success': True,
            'message': 'Registro de carbunco eliminado correctamente'
        })
    except Exception as e:
        app.logger.error(f'Error al eliminar carbunco: {str(e)}')
        conn.rollback()
        cursor.close()
        conn.close()
        return jsonify({
...
@app.route('/eliminar_inseminacion/<int:id>', methods=['DELETE', 'POST'])
@login_required
...
def obtener_genealogia(id):
    try:
        ...
        cursor.execute("""
            SELECT * FROM genealogia WHERE id = %s
        """, (id,))
...
@app.route('/eliminar_genealogia/<int:id>', methods=['DELETE', 'POST'])
@login_required
...
@app.route('/obtener_venta_leche/<int:id>')
@login_required
def obtener_venta_leche(id):
    try:
        ...
        # Asegurarse de que los valores numéricos sean del tipo correcto para JSON
        venta['cantidad_litros'] = float(venta['cantidad_litros'])
        venta['precio_litro'] = float(venta['precio_litro'])
        venta['total_venta'] = float(venta['total_venta'])
        ...
        app.logger.error(f"Error al obtener venta de leche: {str(e)}")
        return jsonify({'success': False, 'message': f'Error al obtener los datos de la venta: {str(e)}'}), 500
...
def editar_venta_leche(id):
    try:
        if request.method == 'POST':
            # Log para depuración
            app.logger.info(f"Datos recibidos en editar_venta_leche: {request.form}")
            app.logger.info(f"ID recibido: {id}")
            
            # Obtener datos del formulario con método get para evitar errores
            fecha = request.form.get('fecha')
            cantidad_litros = float(request.form.get('cantidad_litros', 0))
            ...
            total_venta = cantidad_litros * precio_litro
            
            db = get_db_connection()
            cursor = db.cursor()
            
            # Actualizar los datos básicos sin modificar total_venta (es una columna generada)
            cursor.execute("""
                UPDATE ventas_leche 
                SET fecha = %s,
                    cantidad_litros = %s,
            ...
            # Formatear valores numéricos y fechas para JSON
            if isinstance(venta['cantidad_litros'], (int, float)):
                venta['cantidad_litros'] = float(venta['cantidad_litros'])
            if isinstance(venta['precio_litro'], (int, float)):
                venta['precio_litro'] = float(venta['precio_litro'])
            if isinstance(venta['total_venta'], (int, float)):
                venta['total_venta'] = float(venta['total_venta'])
            if isinstance(venta['fecha'], datetime):
                venta['fecha'] = venta['fecha'].strftime('%Y-%m-%d')
...
def registrar_inseminacion():
    ...
    try:
        ...
        """, (animal_id, fecha_inseminacion, tipo_inseminacion, semental, observaciones))
...
def restablecer_contrasena(token):
    ...
    info_token = sistema_recuperacion.verificar_token(token)
    if not info_token:
    ...
    if request.method == 'POST':
        ...
        # Actualizar la contraseña
        if sistema_recuperacion.actualizar_contrasena(info_token['usuario_id'], password):
            # Marcar el token como usado
            sistema_recuperacion.marcar_token_usado(info_token['token_id'])
            
            flash('Tu contraseña ha sido actualizada correctamente. Ya puedes iniciar sesión.', 'success')
            return redirect(url_for('login'))
        else:
            flash('Hubo un problema al actualizar tu contraseña. Por favor, intenta de nuevo.', 'error')
